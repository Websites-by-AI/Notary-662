...
import { findNotaries } from '../services/geminiService';
...
const NotaryFinder: React.FC<NotaryFinderProps> = ({ keywords, setKeywords, handleApiError, isQuotaExhausted, chatSessions, activeChatId }) => {
    const { t } = useLanguage();
    ...
    const { suggestions, isLoading: areSuggestionsLoading, setSuggestions } = useAISuggestions(
        keywords,
        "Suggest a type of notarial service or a city for finding a notary public office",
    ...
    const handleFind = useCallback(async () => {
        ...
        try {
            const response = await findNotaries(keywords, searchLevel);
            setResults(response);
        } catch (err) {
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <p className="text-xs text-gray-500 mt-2 text-right">{t('notaryFinder.semanticSearchBadge')}</p>
                            </div>
                        </div>
                        ...
                        {results && (
                            <div className="space-y-6">
                               {results.summary && (
                                ...
                                {results.professionals && results.professionals.map((notary, index) => (
                                    <div key={index} className="bg-brand-blue-light p-4 rounded-lg border border-white/10 transform transition-transform hover:scale-[1.02] hover:border-brand-gold/50">
                                        <h3 className="font-bold text-lg text-brand-gold">{notary.name}</h3>
                                        <p className="text-sm text-gray-400 mt-1">{notary.specialty}</p>
                                        <p className="text-sm text-gray-300 mt-3">{notary.summary}</p>
                                        <div className="mt-4 pt-4 border-t border-gray-700 space-y-2 text-sm">
                                            <p><span className="font-semibold text-gray-400">آدرس: </span> {notary.address}</p>
                                            <p><span className="font-semibold text-gray-400">تماس: </span> {notary.contact}</p>
                                            {notary.website && notary.website !== 'N/A' && (
                                                 <p>
                                                    <span className="font-semibold text-gray-400">وب‌سایت: </span>
                                                    <a href={notary.website} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline break-all">
                                                        {notary.website}
                                                    </a>
                                                </p>
                                            )}
                                        ...
                                        <div className="mt-4 pt-4 border-t border-gray-700">
                                            <button
                                                onClick={() => handleSendWhatsApp(notary)}
                                                disabled={!activeDraftContent}
                                                title={activeDraftContent ? t('notaryFinder.sendDraftTooltip') : t('notaryFinder.noDraftToSend')}
